...
class ImgAugTransform(A.ImageOnlyTransform):
    """
    包装 imgaug 增强器的基类，确保与 Albumentations 兼容
    """
    def __init__(self, aug, always_apply=True, p=1.0):
    ...
    def apply(self, img, **params):
        ...
        # 2. imgaug 对 uint8 支持最好，如果输入是 float32 (0-1)，先转回 uint8
        if orig_dtype != np.uint8:
            # 假设 float 图片范围是 [0, 1]
            img = (img * 255).astype(np.uint8)
        
        # 3. 使用 augment_image 确保处理单张图片
        augmented_img = self.aug.augment_image(img)
        
        # 4. 如果原始是 float，转换回去以保持管道一致性
        if orig_dtype != np.uint8:
...
class CustomCloudLayer(ImgAugTransform):
    def __init__(self, seed=35, always_apply=True, p=1.0):
        aug = iaa.CloudLayer(
            ...
            alpha_freq_exponent=-2, sparsity=0.9, density_multiplier=0.5, seed=seed
        )
        super().__init__(aug, always_apply, p)

# 自定义雨 (Rain)
class CustomRain(ImgAugTransform):
    def __init__(self, drop_size=(0.05, 0.1), speed=(0.04, 0.06), seed=None, always_apply=True, p=1.0):
        aug = iaa.Rain(drop_size=drop_size, speed=speed, seed=seed)
        super().__init__(aug, always_apply, p)

# 自定义雪 (Snowflakes)
class CustomSnowflakes(ImgAugTransform):
    def __init__(self, flake_size=(0.5, 0.8), speed=(0.007, 0.03), seed=None, always_apply=True, p=1.0):
        aug = iaa.Snowflakes(flake_size=flake_size, speed=speed, seed=seed)
        super().__init__(aug, always_apply, p)

# 自定义亮度和对比度 (用于 Dark 和 Light，确保和第一段代码逻辑一致)
class CustomBrightness(ImgAugTransform):
    def __init__(self, mul=1.0, add=0, seed=None, always_apply=True, p=1.0):
        aug = iaa.MultiplyAndAddToBrightness(mul=mul, add=add, seed=seed)
        super().__init__(aug, always_apply, p)
...
iaa_weather_list = [
    ...
    # 8. Over-exposure
    # 8. Over-exposure (Light)
    A.Compose([
        CustomBrightness(mul=1.6, add=(0, 30), seed=1992)
    ]),
    
    # 9. Wind
    A.Compose([
        ImgAugTransform(iaa.MotionBlur(15, seed=17))
    ])
]
