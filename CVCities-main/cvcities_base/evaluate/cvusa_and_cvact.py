...
import gc
import copy
from ..trainer import predict,predict_double2query,predict_double2ref,predict2ref,predict2query
...
def evaluate(config,
   ...
   评估函数，用于评估模型在查询数据集上的性能
    ...
    if model == None:
        reference_features, reference_labels = predict(config,model_first,reference_dataloader)
        query_features, query_labels = predict(config, model_first,query_dataloader)
    else:
        reference_features, reference_labels = predict_double2ref(config, model, reference_dataloader,model_first=model_first)
        query_features, query_labels = predict_double2query(config, model, query_dataloader,model_first=model_first)
...
def calc_sim(config,
    ...
    if model==None:
        ...
        query_features, query_labels = predict2query(config, model, query_dataloader)
